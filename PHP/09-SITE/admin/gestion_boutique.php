...
if(isset($_GET['action']) && $_GET['action'] == 'suppression' && isset($_GET['id_produit'])){
    ...
    //on commence par sélectionner la photo en base pour pouvoir supprimer le fichier physique:
    $resultat = executeRequete("SELECT photo FROM produit WHERE id_produit = :id_produit", array(':id_produit' => $_GET['id_produit']));
    
    $produit_a_supprimer = $resultat->fetch(PDO::FETCH_ASSOC); //array associatif dont le nom de l'indice correspond au champ présent dans le SELECT de la requête ci_dessus
    
    //puis je détermine le
    $chemin_photo_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . $produit_a_supprimer['photo']; //chemin complet de la photo à supprimer dans le dossier photo
    
    if(!empty($produit_a_supprimer['photo']) && file_exists($chemin_photo_a_supprimer)){
        //vérifier que la photo n'a pas déjà été supprimée sinon erreur (file_exists)
...
/*    debug($_POST);*/
    ...
    if(!empty($_FILES['photo']['name'])){
        ...
        $photo_physique = $_SERVER['DOCUMENT_ROOT'] . $photo_bdd; //représente le chemin complet depuis le 'localhost'/document_root, où est enregistré le fichier photo physique sur le serveur.
        
        copy($_FILES['photo']['tmp_name'], $photo_physique); //enregistre le fichier photo qui est temporairement dans $_FILES['photo']['tmp_name'], dans le répertoire dont le chemin est photophysique.
    ...
    executeRequete("REPLACE INTO produit (id_produit, reference, categorie, titre, description, couleur, taille, public, photo, prix, stock)
                  ...
                  array(':id_produit' => $_POST['id_produit'], ':reference' => $_POST['reference'], ':categorie' => $_POST['categorie'], ':titre' => $_POST['titre'], ':description' => $_POST['description'], ':couleur' => $_POST['couleur'], ':taille' => $_POST['taille'], ':public' => $_POST['public'], ':photo' => $photo_bdd, ':prix' => $_POST['prix'], ':stock' => $_POST['stock']));
    $contenu .= '<div class= "bg-success">Le produit a été enregistré.</div>';
    $_GET['action'] = 'affichage'; //va rediriger direct après un enregistrement sur la page affichage de produit. Déclenche l'affichage de la page html avec tous les produits. Tout ça est dans la condition (!$_POST)
    
} //fin du if(!empty($_POST))


//2- Lien affichage et ajout de produit:
...
if(isset($_GET['action']) && $_GET['action'] == 'affichage'){
    ...
    $resultat = executeRequete("SELECT * FROM produit"); //on sélectionne 
    tous les produits
            ...
            while($ligne = $resultat->fetch(PDO::FETCH_ASSOC)){
               /* debug($ligne);*/
                    ...
                    $contenu .= '<td>
                                    ...
                                    <a href="?action=suppression&id_produit='. $ligne['id_produit'] .'" onclick="return(confirm(\'Êtes-vous sûr de vouloir supprimer ce produit?\'))">supprimer</a>
                                </td>';
                    $contenu .= '</tr>';
            }
...
if(isset($_GET['id_produit'])){
    ...
    $produit_actuel = $resultat->fetch(PDO::FETCH_ASSOC); //pas de boucle while car un seul produit vu qu'on demande l'id
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <!--value 0 on le force pour qu'il se comporte en insert si l'id n'existe pas et non update au debut puis ligne php -->
    
    
    <label for="reference">Référence</label><br>
    <input type="text" name="reference" id="reference" value="<?php if(isset($produit_actuel)){ echo $produit_actuel['reference']; } ?>"><br><br>
    <!--copier colle de la value mais on vire le else qui servait pour l'id-->
    ...
    <input type="file" id="photo" name="photo"><br><br> <!--va de paire avec l'attribut enctype de <form>, permet d'uploader un fichier et de remplir la superglobale $_FILES-->
